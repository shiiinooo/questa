...
from src.widgets.feedback_system import FeedbackSystem, FeedbackMessage, FeedbackType
...
class TestFeedbackSystem:
    """Test the FeedbackSystem widget."""
    
    @pytest.fixture
    def feedback_system(self):
        """Create a FeedbackSystem widget."""
        return FeedbackSystem(max_messages=5)
    
    def test_feedback_message_creation(self):
        """Test creating feedback messages."""
        ...
        assert message.feedback_type == FeedbackType.SUCCESS
        ...
        assert message.id.startswith("feedback_")
    
    def test_add_message(self, feedback_system):
        """Test adding messages to feedback system."""
        message = FeedbackMessage("Test message", FeedbackType.INFO)
        
        feedback_system.add_message(message)
        
        assert len(feedback_system.messages) == 1
        assert feedback_system.message_count == 1
        assert message.id in feedback_system.message_widgets
    
    def test_remove_message(self, feedback_system):
        """Test removing messages from feedback system."""
        message = FeedbackMessage("Test message", FeedbackType.INFO)
        feedback_system.add_message(message)
        
        feedback_system.remove_message(message.id)
        
        assert len(feedback_system.messages) == 0
        assert feedback_system.message_count == 0
        assert message.id not in feedback_system.message_widgets
    
    def test_max_messages_limit(self, feedback_system):
        """Test that message count is limited to max_messages."""
        # Add more messages than the limit
        for i in range(7):
            message = FeedbackMessage(f"Message {i}", FeedbackType.INFO)
            feedback_system.add_message(message)
        
        # Should only keep the last 5 messages
        assert len(feedback_system.messages) == 5
        assert feedback_system.message_count == 5
        
        # Check that the oldest messages were removed
        message_texts = [msg.message for msg in feedback_system.messages]
        assert "Message 0" not in message_texts
        assert "Message 1" not in message_texts
        assert "Message 6" in message_texts
    
    def test_clear_messages(self, feedback_system):
        ...
        feedback_system.clear_messages()
        
        assert len(feedback_system.messages) == 0
        assert feedback_system.message_count == 0
        assert len(feedback_system.message_widgets) == 0
    
    def test_get_message_count_by_type(self, feedback_system):
        """Test getting message count by type."""
        feedback_system.add_message(FeedbackMessage("Success 1", FeedbackType.SUCCESS))
        feedback_system.add_message(FeedbackMessage("Success 2", FeedbackType.SUCCESS))
        feedback_system.add_message(FeedbackMessage("Error 1", FeedbackType.ERROR))
        feedback_system.add_message(FeedbackMessage("Warning 1", FeedbackType.WARNING))
        
        assert feedback_system.get_message_count_by_type(FeedbackType.SUCCESS) == 2
        assert feedback_system.get_message_count_by_type(FeedbackType.ERROR) == 1
        assert feedback_system.get_message_count_by_type(FeedbackType.WARNING) == 1
        assert feedback_system.get_message_count_by_type(FeedbackType.INFO) == 0
    
    def test_has_errors_and_warnings(self, feedback_system):
        """Test checking for errors and warnings."""
        assert not feedback_system.has_errors()
        assert not feedback_system.has_warnings()
        
        feedback_system.add_message(FeedbackMessage("Error", FeedbackType.ERROR))
        assert feedback_system.has_errors()
        assert not feedback_system.has_warnings()
        
        feedback_system.add_message(FeedbackMessage("Warning", FeedbackType.WARNING))
        assert feedback_system.has_errors()
        assert feedback_system.has_warnings()
    
    def test_convenience_methods(self, feedback_system):
        """Test convenience methods for showing different message types."""
        # Test show_success
        success_id = feedback_system.show_success(
            "Success message",
            title="Success",
            details={'xp': 30},
            duration=2.0
        ...
        success_msg = next(msg for msg in feedback_system.messages if msg.id == success_id)
        assert success_msg.feedback_type == FeedbackType.SUCCESS
        assert success_msg.message == "Success message"
        assert success_msg.title == "Success"
        assert success_msg.details['xp'] == 30
        assert success_msg.duration == 2.0
        
        # Test show_error
        error_id = feedback_system.show_error(
            "Error message",
            title="Error",
            duration=0  # Permanent
        )
        
        error_msg = next(msg for msg in feedback_system.messages if msg.id == error_id)
        assert error_msg.feedback_type == FeedbackType.ERROR
        assert error_msg.duration == 0
        
        # Test show_progress
        progress_id = feedback_system.show_progress(
            "Loading...",
            progress=50,
            title="Progress"
        )
        
        progress_msg = next(msg for msg in feedback_system.messages if msg.id == progress_id)
        assert progress_msg.feedback_type == FeedbackType.PROGRESS
        assert progress_msg.details['progress'] == 50
    
    def test_update_progress_message(self, feedback_system):
        """Test updating progress messages."""
        progress_id = feedback_system.show_progress("Loading...", progress=25)
        
        feedback_system.update_progress_message(progress_id, 75, "Almost done...")
        
        progress_msg = next(msg for msg in feedback_system.messages if msg.id == progress_id)
        assert progress_msg.details['progress'] == 75
        assert progress_msg.message == "Almost done..."
...
class TestIntegratedErrorHandling:
    ...
    def test_feedback_system_integration(self):
        """Test integration of feedback system with error handling."""
        feedback_system = FeedbackSystem()
        
        # Simulate various operations with feedback
        
        # Success feedback
        success_id = feedback_system.show_success(
            "Task created successfully!",
            details={'xp_reward': 30, 'level_up': False}
        )
        assert feedback_system.get_message_count_by_type(FeedbackType.SUCCESS) == 1
        
        # Error feedback
        error = TaskValidationError("Title cannot be empty")
        user_error = ErrorHandler.handle_validation_error(error)
        
        error_id = feedback_system.show_error(
            user_error.message,
            title=user_error.title,
            details={'technical_details': user_error.technical_details}
        )
        assert feedback_system.has_errors()
        
        # Warning feedback
        warning_id = feedback_system.show_warning(
        ...
        # Update progress
        feedback_system.update_progress_message(progress_id, 50, "Halfway done...")
        feedback_system.update_progress_message(progress_id, 100, "Complete!")
        
        # Verify all messages are tracked
        assert len(feedback_system.messages) == 4
        assert feedback_system.message_count == 4